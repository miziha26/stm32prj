...
//	 delay_ms(10);		
// 	 // 初始化 TIM3，设置为 3 秒超时
//   //TIM3_Init(10000);  // 初始化 TIM3，并设置为 3 秒的时间（单位 ms）
//	while(1)
//	{	
...
{
    SysTick_Config(SystemCoreClock / 1000); // 配置 SysTick 每 1 毫秒中断一次
}
...
{	
    ...
    Init_UART1(9600);
    start_time = millis();  // 获取当前时间，假设 millis() 返回当前时间的毫秒数

    if (!DHT11_Init())
    {
        printf(" Error! T DHT11 HAS NO RESPOND...\n");
        //return; // 如果 DHT11 初始化失败，直接返回
    }
...
void run()
{
    ...
    //u32 test_addr = 0x00010000;
	//u8 read_data[16];
	W25Q128_Read_Bytes(test_addr, read_data, 16);
	int num = read_data[0];
	while(1)
	{	
		// 获取指定通道的平均ADC值，这里是PA4 
		...
		//if(smoke_concentration > num)//烟雾浓度过高，触发报警
		//{
			//BEEP_ON;
			//LED_ALL_ON;
			//opendoor();
		...
		if (USART2_GetRxFlag() == 1)  // 判断是否收到数据
		{
			...
			switch (Direction)
			{
				...
				{
					LED1_ON;
					OLED_ClearArea(1, 1, 125, 61);
					OLED_ShowChinese(16, 16, "灯已打开");
					OLED_Update();
					break;  
				}
				case 'b': // 关卧室灯
				...
				case 'i': // 测温湿度
				{
					if (DHT11_Read_Data(&tem1, &hum1))
					{
						...
						OLED_ShowChinese(20, 27,"湿度");
						sprintf(dis1, ": %d", tem1);
						OLED_ShowString(54, 10, dis1, OLED_8X16);
						OLED_Update();
						sprintf(dis1, ": %d", hum1);
						OLED_ShowString(54, 27, dis1, OLED_8X16);
						OLED_Update();
					}
					else
			        {
				...
				case 'j': // 测烟雾浓度
				{
					...
					// 将结果显示到 OLED
					OLED_ClearArea(1, 1, 125, 61);  // 清除上一轮显示内容
					OLED_ShowChinese(2, 16, "烟雾");
					// 格式化烟雾浓度值
					sprintf(buffer, ": %.2fppm", smoke_concentration);
					OLED_ShowString(34, 16, buffer,OLED_8X16); 
					OLED_Update();
					break;
				}
				case 'k': // 报警
...
void adjust()
{
	...
	{
		write_data[0]=50;
		W25Q128_Write_Bytes(test_addr, write_data, 16);
	}    	
	W25Q128_Read_Bytes(test_addr, read_data, 16);
	int num = read_data[0];
	OLED_ShowNum(75, 20, num, 2, OLED_8X16);
    OLED_Update();   
	while(1)
	{
		if(Key_Status(KEY3) == KEY_DOWN)//加1
		{
			delay_ms(15);//跳过按下抖动
			if(Key_Status(KEY3) == KEY_DOWN)//看是否被调戏了 是否真的按下
			{
		...
		if(Key_Status(KEY2) == KEY_DOWN)//减1
		{
			delay_ms(15);//跳过按下抖动
			if(Key_Status(KEY2) == KEY_DOWN)//看是否被调戏了 是否真的按下
			{
		...
		if(Key_Status(KEY1) == KEY_DOWN)//减1
		{
			delay_ms(15);//跳过按下抖动
			if(Key_Status(KEY1) == KEY_DOWN)//看是否被调戏了 是否真的按下
			{
...
void showtable()
{
	...
	char dis[16]="1.";
	char dis0[16]="2.";
	OLED_ShowChinese(31, 0, "安全管家");
	OLED_ShowString(24, 20, dis, OLED_8X16);
	OLED_ShowString(24, 40, dis0, OLED_8X16);
	OLED_ShowChinese(37, 20, "开始工作");
	OLED_ShowChinese(37, 40, "调节阈值");
	OLED_DrawRectangle(0, 0, 127, 63, OLED_UNFILLED);
	OLED_Update();
	while(1)
	{
		OLED_ShowChinese(31, 0, "安全管家");
		...
		OLED_DrawRectangle(0, 0, 127, 63, OLED_UNFILLED);
		OLED_Update();
		if(Key_Status(KEY2) == KEY_DOWN)//开始工作
		{
			delay_ms(15);//跳过按下抖动
			if(Key_Status(KEY2) == KEY_DOWN)//看是否被调戏了 是否真的按下
			{
		...
		if(Key_Status(KEY3) == KEY_DOWN)//调节阈值
		{
			delay_ms(15);//跳过按下抖动
			if(Key_Status(KEY3) == KEY_DOWN)//看是否被调戏了 是否真的按下
			{
