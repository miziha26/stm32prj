...
void run()
{
	...
	{	
		...
		// 计算烟雾浓度 
		smoke_concentration = 0.000806*avg_adc_value/(10/14.7)*20;
		if(smoke_concentration > 70)//烟雾浓度过高，触发报警
		{
			BEEP_ON;
			LED_ALL_ON;
			opendoor();
			...
			switch (Direction)
			{
				...
				case 'i': // 测温湿度
				{
					if (DHT11_Read_Data(&tem1, &hum1))
					{
						...
						sprintf(dis1, ": %d", tem1);
						OLED_ShowString(34, 0, dis1, OLED_8X16);
						OLED_Update();
						sprintf(dis1, ": %d", hum1);
						OLED_ShowString(34, 17, dis1, OLED_8X16);
						OLED_Update();
					}
					else
			        {
				...
				case 'j': // 测烟雾浓度
				{
					// 串口打印结果
					printf("Smoke Value: %.2f ppm\n", smoke_concentration);
					if(smoke_concentration > 50)//烟雾浓度过高，触发报警
					{
						BEEP_ON;
						LED_ALL_ON;
						opendoor();
...
void adjust()
{
	...
	OLED_ShowString(75, 20, read_data, OLED_8X16);
